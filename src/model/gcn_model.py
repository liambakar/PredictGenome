...
class FeatureProcessor(nn.Module):
    """
    处理每个特征的变长序列，将其转换为固定长度的embedding
    """
...
class GCNGenomicModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.feature_processor = FeatureProcessor(embedding_dim=feature_embedding_dim)
    ...
    def forward(self, feature_list):
        ...
        for feature in feature_list:
            embedding = self.feature_processor(feature)  # (batch_size, embedding_dim)
...
class CorrelationBasedGCNModel(GCNGenomicModel):
    ...
    def get_adjacency_matrix(self, batch_size, feature_embeddings=None):
        ...
        for b in range(batch_size):
            ...
            # Add self-connections
            corr_matrix = corr_matrix + torch.eye(self.num_features, device=corr_matrix.device)
