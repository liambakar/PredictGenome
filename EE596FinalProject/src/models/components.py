...
class FeedForwardEnsemble(nn.Module):
    def __init__(self, dim, mult=1, dropout=0., num=16):
        ...
        self.net = nn.ModuleList(
            [FeedForward(dim, mult, dropout) for _ in range(num)])
...
class MMAttention(nn.Module):
    ...
    def forward(self, x, mask=None, return_attn=False):
        ...
        if self.attn_mode == 'full':  # H->P, P->H, P->P, H->H
            ...
            attn_histology_pathways = torch.cat(
                (cross_attn_histology, attn_histology), dim=-1).softmax(dim=-1)
...
def predict_surv(self, dataset,  use_cuda=True, permute=False):
    ...
    for i in tqdm(range(len(dataset))):
        batch = dataset.__getitem__(i)
        data, label, censorship, time = batch['img'].unsqueeze(dim=0), batch['label'].unsqueeze(
            dim=0), batch['censorship'].unsqueeze(dim=0), batch['survival_time'].unsqueeze(dim=0)
    ...
    if loss_fn is not None and label is not None:
        if isinstance(loss_fn, NLLSurvLoss):
            surv_loss_dict = loss_fn(
                logits=logits, times=label, censorships=censorship)
            ...
            surv_loss_dict = loss_fn(
                z=logits, times=label, censorships=censorship)
