...
class OT_Attn(nn.Module):
    ...
    def OT(self, weight1, weight2):
        ...
        if self.impl == "pot-uot-l2":
            a, b = torch.tensor(ot.unif(weight1.size()[0])).cuda(
            ), torch.tensor(ot.unif(weight2.size()[0])).cuda()
    ...
    def forward(self, x, y):
        ...
        pi = [self.OT(self.normalize_feature(x[idx]),
                      self.normalize_feature(y[idx])) for idx in range(len(x))]
...
def construct_proto_embedding(path_proj_dim, append_embed='modality', numOfproto_histo=16, numOfproto_omics=50):
    ...
    if append_embed == 'modality':  # One-hot encoding for two modalities
        ...
        histo_embedding = torch.tensor([[[1, 0]]]).repeat(
            1, numOfproto_histo, 1)  # (1, numOfproto, 2)
        gene_embedding = torch.tensor([[[0, 1]]]).repeat(
            1, numOfproto_omics, 1)  # (1, len(omic_sizes),2 )
        ...
        # (1, numOfproto, numOftotalproto)
        histo_embedding = embedding[:, :numOfproto_histo, :]
        # (1, len(omic_sizes), numOftotalproto)
        ...
        histo_embedding = torch.nn.Parameter(torch.randn(
            1, numOfproto_histo, append_dim), requires_grad=True)
        gene_embedding = torch.nn.Parameter(torch.randn(
            1, numOfproto_omics, append_dim), requires_grad=True)
...
class coattn(nn.Module):
    def __init__(
        ...
        if self.histo_model == 'panther':  # Uses prob/mean/cov
            self.path_proj_net = nn.Sequential(
                nn.Linear(self.histo_in_dim * 2 + 1, path_proj_dim))
        else:
            self.path_proj_net = nn.Sequential(
                nn.Linear(self.histo_in_dim, path_proj_dim))
        ...
        if self.num_coattn_layers == 0:
            ...
            if self.net_indiv:  # Individual MLP per prototype
                ...
                feed_forward = FeedForward(
                    out_dim, self.out_mult, dropout=dropout)
            ...
            # If we want to use only single modality + self-attention
        ...
        histo_final_dim = out_dim_final * \
            self.numOfproto if self.histo_agg == 'cat' else out_dim_final
    ...
    def forward_no_loss(self, x_path, x_omics, return_attn=False):
        ...
        if self.histo_embedding is not None:    # Append histo prototype encoding
            ...
            for idx in range(len(h_path)):
                arr.append(
                    torch.cat([h_path[idx:idx + 1], self.histo_embedding.to(device)], dim=-1))
        ...
        if return_attn:
            with torch.no_grad():
                _, attn_pathways, cross_attn_pathways, cross_attn_histology = self.coattn[0](
        ...
        if self.histo_model == 'mil':
            # For non-prototypes, we just take the mean
        ...
        if self.modality == 'histo':    # Just use histo for prediction
            ...
            # ---> both branches
            embedding = torch.cat(
                [paths_postSA_embed, wsi_postSA_embed], dim=1)
...
class coattn_mot(nn.Module):
    ...
    def forward_no_loss(self, x_path, x_omics, return_attn=False):
        ...
        tokens = torch.cat([h_path_coattn, h_omic_coattn],
                           dim=1)  # (B, N_p+N_h, d)
        tokens = self.identity(tokens)
...
class SurvPath(nn.Module):
    def __init__(
        ...
        self.num_classes = num_classes
        self.path_proj_net = nn.Sequential(
            nn.Linear(self.path_dim, self.path_proj_dim))
        ...
        self.coattn = nn.Sequential(
            *[cross_attender, feed_forward, layer_norm])

        histo_final_dim = self.path_proj_dim // 2
        gene_final_dim = self.path_proj_dim // 2

        self.classifier = nn.Linear(
            histo_final_dim + gene_final_dim, self.num_classes)
