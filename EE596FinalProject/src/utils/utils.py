...
def parse_model_name(model_name, ckpt=None, inference_prec=None):
    ...
    if ckpt is None:
        ...
        if ckpt.isnumeric():
            ckpt = int(ckpt)
            exp = '.'.join(exp_parsed[:-1]) + \
                '_'.join(exp_parsed[-1].split('_')[:-1])
        else:
            ckpt = -1
    else:
...
def read_splits(args, fold_idx=None):
    ...
    for split in split_names:
        splits_csvs[split] = {}
        splits_csvs[split]['histo'] = _read_histo(
            args.split_dir, split, fold_idx)
...
def get_lr_scheduler(epochs,
                     ...
                     accum_steps=32):
    assert not (warmup_steps > 0 and warmup_epochs >
                0), "Cannot have both warmup steps and epochs"
    if warmup_steps > 0:
    ...
    if scheduler_name == 'constant':
        ...
        lr_scheduler = get_cosine_schedule_with_warmup(optimizer=optimizer,
                                                       num_warmup_steps=warmup_steps,
                                                       num_training_steps=(
                                                           len(dataloader) // accum_steps * epochs),
                                                       )
...
def get_optim(model, opt="adamW", lr=1e-3, wd=1e-5):
    ...
    if opt == "adamW":
        ...
        optimizer = torch.optim.RAdam(parameters, lr=lr, weight_decay=wd)
    else:
